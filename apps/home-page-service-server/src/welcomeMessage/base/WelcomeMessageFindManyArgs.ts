/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { WelcomeMessageWhereInput } from "./WelcomeMessageWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { WelcomeMessageOrderByInput } from "./WelcomeMessageOrderByInput";

@ArgsType()
class WelcomeMessageFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => WelcomeMessageWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => WelcomeMessageWhereInput, { nullable: true })
  @Type(() => WelcomeMessageWhereInput)
  where?: WelcomeMessageWhereInput;

  @ApiProperty({
    required: false,
    type: [WelcomeMessageOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [WelcomeMessageOrderByInput], { nullable: true })
  @Type(() => WelcomeMessageOrderByInput)
  orderBy?: Array<WelcomeMessageOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { WelcomeMessageFindManyArgs as WelcomeMessageFindManyArgs };
